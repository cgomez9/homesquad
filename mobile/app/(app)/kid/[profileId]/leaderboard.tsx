...
// pedestal palette by podium place
const PED = {
...
export default function KidLeaderboardScreen() {
  ...
  const { data: familyId } = useQuery({
    queryKey: ['kid-family-id-leaderboard'],
      ...
      <View style={styles.content}>
        ...
        {!isLoading && isSolo && (
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
            ...
            <SoloCard row={sorted[0]} stars={starsLabel(starsOf(sorted[0]))} me={sorted[0].profile_id === profileId} />
          </ScrollView>
        )}
  ...
  return (
    ...
    >
      ...
      <View
        style={[
          styles.av,
          { width: avSize, height: avSize, borderRadius: avSize / 2, backgroundColor: a.bg },
          me && styles.avMe,
        ]}
      >
        <Text style={{ fontSize: avSize * 0.5 }}>{a.emoji}</Text>
      </View>
      <Text style={[styles.colName, me && styles.meText]} numberOfLines={1}>
        {row.display_name}
      </Text>
...
function RestRow({
  ...
  return (
    ...
    >
      ...
      <View style={[styles.rowAv, { backgroundColor: a.bg }, me && styles.avMe]}>
        <Text style={styles.rowEmoji}>{a.emoji}</Text>
      </View>
      <Text style={[styles.rowName, me && styles.meText]} numberOfLines={1}>
        {row.display_name}
      </Text>
...
function SoloCard({ row, stars, me }: { row: LeaderboardRow; stars: string; me: boolean }) {
  const { colors } = useTheme();
  const styles = useMemo(() => makeStyles(colors), [colors]);
  const a = AVATARS[(row.avatar_id ?? 1) as AvatarId] ?? AVATARS[1];
  return (
    <View style={styles.soloCard}>
      <View style={[styles.soloAv, { backgroundColor: a.bg }, me && styles.avMe]}>
        <Text style={styles.soloEmoji}>{a.emoji}</Text>
      </View>
      <Text style={[styles.soloName, me && styles.meText]}>{row.display_name}</Text>
...
const makeStyles = (colors: Palette) =>
  ...
  rowMe: { backgroundColor: '#FFF1F2', borderWidth: 2, borderColor: colors.accent },
