...
-- First, update star_ledger constraint to allow 'test' reason (for testing)
alter table public.star_ledger
  drop constraint star_ledger_reason_check;

alter table public.star_ledger
  add constraint star_ledger_reason_check
    check (reason in ('chore_approved','redemption','manual_grant','manual_revoke','test'));
